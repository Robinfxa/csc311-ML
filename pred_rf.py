"""
CSC311 ML Challenge - Random Forest Inference
==============================================
Loads the trained Random Forest model from rf_results/ and performs
prediction using the same preprocessing pipeline as pred_lr.py.

Usage:
    python pred_rf.py                          # self-test on training CSV
    from pred_rf import predict_all
...
# ── column aliases (same as explore_data.py / pred_lr.py) ─────────────
COL_ALIASES = {
...
# ── Load model & scaler (done once at import time) ─────────────────────
def _load_params():
    model       = joblib.load(os.path.join(RF_DIR, "rf_model.joblib"))
    scaler_mean = np.load(os.path.join(DATA_DIR, "scaler_mean.npy"))
    scaler_std  = np.load(os.path.join(DATA_DIR, "scaler_std.npy"))

    label_map = {}
    with open(os.path.join(RF_DIR, "label_map.txt"), "r") as f:
        for line in f:
            idx, name = line.strip().split(",", 1)
            label_map[int(idx)] = name

    return model, scaler_mean, scaler_std, label_map


_model, _scaler_mean, _scaler_std, _label_map = _load_params()


# ═══════════════════════════════════════════════════════════════════════
#  PREPROCESSING (mirrors explore_data.py / pred_lr.py)
# ═══════════════════════════════════════════════════════════════════════
...
def preprocess(df):
    ...
    # Fill NaN in numeric/ordinal with in-batch median (fallback: 3.0)
    ...
    # Assemble feature matrix in fixed order
...
def predict_all(filename):
    ...
    # Standardize using saved scaler params (same scaler as LR)
    X_scaled = (X - _scaler_mean) / _scaler_std

    pred_indices = _model.predict(X_scaled)
...
if __name__ == "__main__":
    ...
    print(f"Testing pred_rf on: {csv_path}")
